...
const curatedItems: TickerItem[] = [
  ...
  { tag: "OKC", text: "DEVON ENERGY CENTER — 844 FT — TALLEST IN OKLAHOMA" },
  ...
  { tag: "OKC", text: "OKLAHOMA CITY NATIONAL MEMORIAL — APRIL 19, 1995" },
  ...
  { tag: "THUNDER", text: "OKLAHOMA CITY THUNDER — EST. 2008 — NBA WESTERN CONF" },
...
function getTimeItem(): TickerItem {
  ...
  const timeStr = now.toLocaleTimeString("en-US", {
    hour: "numeric", minute: "2-digit", timeZone: "America/Chicago", hour12: true,
  }).toUpperCase();
...
export function LiveTicker() {
  ...
  const fetchWeather = useCallback(async () => {
    try {
      ...
      setWeatherItem({ tag: "WEATHER", text: `${temp}°F ${condition} — OKLAHOMA CITY` });
    } catch {
      setWeatherItem({ tag: "WEATHER", text: "OKLAHOMA CITY WEATHER — CHECK LOCAL" });
    }
  }, []);
  ...
  const allItems: TickerItem[] = [timeItem, ...(weatherItem ? [weatherItem] : []), ...curatedItems];
      ...
      <div className="marquee-track">
        {doubled.map((item, i) => (
          <span key={i} className="whitespace-nowrap mx-5 inline-flex items-center gap-2">
            ...
            <span className="text-[11px] uppercase tracking-[0.12em] font-semibold text-white/40">{item.text}</span>
